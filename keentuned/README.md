...
VERSION_NUM     = 2.0.1                     ; Record the version number of keentune
...
## Docker command
```
docker build -t keentune:base . -f Dockerfile-base
docker build -t keentune:keentuned . -f Dockerfile-keentuned
docker build -t keentune:brain .

docker run --name keentund -d keentune:base -p 9870:9870 -p 9871:9871
docker run --name brain -d keentune:base port 9872:9872
docker run --name bench -d keentune:base
docker run --name keentund -d keentune:base
docker run --name target -d keentune:base


docker run --name brain -d keentune:brain port 9872:9872
docker run --name keentund -d keentune:keentuned
docker run --name keentund -d keentune:keentuned




docker run --name brain -d keentune:brain -v /volume/conf:etc/keentune/conf -v /volume/log/keentuned:/var/log/keentune


docker run --name my-keentun -d keentune:v1.0
docker exec -it 26e299e0d8d5 /bin/bash

docker exec -it aee29037ea69 /bin/bash  # bench
docker exec -it 076fa74c4de2 /bin/bash  # brain
docker exec -it e97b718d9704 /bin/bash  # target
docker exec -it 28e98b6dfc96 /bin/bash  # keentuned
